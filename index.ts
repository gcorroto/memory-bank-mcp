...
server.tool(
  ...
  manageAgentsToolDefinition.description,
  {
    ...
    action: z.enum(["register", "update_status", "claim_resource", "release_resource", "get_board", "complete_task", "claim_task", "get_task_details"]).describe("Acción a realizar"),
    ...
    taskId: z.string().optional().describe("ID de la tarea para complete_task, claim_task o get_task_details (ej: 'EXT-123456', 'TASK-789012')."),
  ...
  async (args) => {
    ...
    if ((args.action === 'complete_task' || args.action === 'claim_task' || args.action === 'get_task_details') && !args.taskId) {
